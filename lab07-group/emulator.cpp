...
int parse_instr(int line, char* ftok, instr* imem, int memoff, label_loc* labels, source* src) {
	...
	if ( pscnt > 0 ) {
		...
		switch( op ) {
			...
			//instruction added
			case ORN:case ROL:case ROR:case BCLR:case BEXT:
			...
			case CTZ:case REV8:
				if (!o1 || !o2 || o3 || o4) print_syntax_error(line, "Invalid format");
					...
					i->a2.reg = parse_reg(o2, line);
				return 1;
			case RORI:case BCLRI:case BEXTI:
				if (!o1 || !o2 || !o3 || o4) print_syntax_error(line, "Invalid format");
					i->a1.reg = parse_reg(o1, line);
					i->a2.reg = parse_reg(o2, line);
			...
			case JALR:
				...
				return 1;
			case MUL: case ADD: case SUB: case SLT: case SLTU: case AND: case OR: case XOR: case SLL: case SRL: case SRA:
				if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line, "Invalid format" );
				i->a1.reg = parse_reg(o1, line);
				i->a2.reg = parse_reg(o2, line);
				i->a3.reg = parse_reg(o3, line);
			...
			case LB: case LBU: case LH: case LHU: case LW: case SB: case SH: case SW:
				...
				parse_mem(o2, &i->a2.reg, &i->a3.imm, 12, line);
				return 1;
			case ADDI: case BSETI: case SLTI: case SLTIU: case ANDI: case ORI: case XORI: case SLLI: case SRLI: case SRAI:
				if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line, "Invalid format" );
