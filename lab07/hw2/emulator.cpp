...
void execute(uint8_t* mem, instr* imem, label_loc* labels, int label_count, bool start_immediate) {
	...
	while(!dexit) {
		...
		switch (i.op) {
			...
			case ROL: rf[i.a1.reg] = (rf[i.a2.reg]<<(rf[i.a3.reg]&0x1F)) | (rf[i.a3.reg]>>(32-rf[i.a3.reg]&0x1F)); break;
			
			// let shamt = 	if   xlen == 32
			// 				then X(rs2)[4..0]
			// 				else X(rs2)[5..0];
			// X(rd) = (X(rs1) >> shamt) | (X(rs2) << (xlen - shamt));
			case ROR: rf[i.a1.reg] = (rf[i.a2.reg]>>(rf[i.a3.reg]&0x1F)) | (rf[i.a3.reg]<<(32-rf[i.a3.reg]&0x1F)); break;
			
			// let shamt = if   xlen == 32
			// 				then shamt[4..0]
			// 				else shamt[5..0];
			//  X(rd) = (X(rs1) >> shamt) | (X(rs2) << (xlen - shamt));
			case RORI: rf[i.a1.reg] = (rf[i.a2.reg]>>(rf[i.a3.reg]&0x1F)) | (rf[i.a3.reg]<<(32-rf[i.a3.reg]&0x1F)); break;
